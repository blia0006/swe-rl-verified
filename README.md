...
**7 个 step 出现 `score_max=1.0`**，意味着模型写出了让 F2P 全过、P2P 无回归的**正确修复** ——
这是上一轮从未做到的。search/replace 表示确实解除了主要瓶颈。
...
**如实说明**：第 4 条未达成。但相比上一轮，模型从"从未修对过任何题"变为
"22.6% 的 step 出现满分"，主要瓶颈（unified diff 行号计算）已解除；
