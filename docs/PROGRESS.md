...
> **⚠️ 09-03 追记（重要勘误）**：下文 Phase 0 第 2 项"不用 k8s：宿主机 `ctr`
> 直起 GPU 容器"是 **09-02 设计阶段**的决策，当时判断 pod 链路脆（kubeconfig
> 会过期）而选择裸容器。明确②「GPU 训练须部署在 TKE」的要求后，
> 09-03 已整改：TKE Pod 重新部署并验证通过（commit `3520ac9`/`df4ff5c`），
> 且**正式 GRPO 训练（3B，192 步）已改为经 `kubectl exec` 在 TKE Pod 容器内
> 跑完**，详见 `docs/TKE-GRPO训练报告.md`。下文保留作历史决策记录，
> 但"运行方式"一行已不代表最终交付状态。
...
| 运行方式（设计阶段，09-02） | 宿主机 `ctr` 直起容器 + 本地终端监督 | 见 Phase 0 第 1、2 项；**09-03 已整改为 TKE Pod 内 `kubectl exec` 执行，见 `docs/TKE-GRPO训练报告.md`** |
...
|---|---|---|
| **组内出现满分 1.0 的 step** | **1/55 = 1.8%**（step 47） | **7/31 = 22.6%** |
| 均值 >0.2 的 step | 1/55 | 0/31（分布形态变化，见 README §7.3 解读） |
| 样本级满分次数 | 0 | **15** |
